...
fprintf('=================== INTEGRIDAD SHA-2 (SHA-256) ===================\n');
fprintf('\nSeleccione el archivo en texto plano para calcular SHA-256...\n');
% Abrimos una ventana emergente para seleccionar el archivo a abrir:
[NombreArchivo, Ruta]= uigetfile({'*.txt'});
% Con  '*.txt'  solo se visualizará los archivos de tipo .txt
ID_Archivo = fopen([Ruta NombreArchivo],'rt');
% opción rt (r: Open file for Reading, t: Text Mode)
txt_claro  = fread(ID_Archivo);
% De códigos pasamos a caracteres:
txt_claro_a_caracteres = char(txt_claro);
fclose(ID_Archivo); %cerramos el archivo.
% Mostramos el texto extraído del archivo:
fprintf('\n=========================================================\n');
fprintf('El archivo "');
fprintf('%c',NombreArchivo);
fprintf('" contiene el siguiente texto:');
% Transponemos el arreglo txt_claro_a_caracteres para que se muestre
% correctamente:
mensaje = txt_claro_a_caracteres'
% Fin de la muestra del texto:
fprintf('\n=========================================================\n');
fprintf('\n Calculando el hash SHA-2 (variante SHA-256). Espere...\n');

%% COMENZAMOS EL CONTEO DEL TIEMPO:
tic
...
tiempo_computo = toc; % Almacenamos el final del conteo del tiempo.
fprintf('Tiempo de cómputo:\n\t\t\t%d segundos\n\n',tiempo_computo); %imprimimos
